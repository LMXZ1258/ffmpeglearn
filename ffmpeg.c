...
int        nb_input_streams = 0;    // 输出数量
InputFile   **input_files   = NULL; // 输入文件
int        nb_input_files   = 0;    // 有多少个输入文件

OutputStream **output_streams = NULL;
int         nb_output_streams = 0;  // 输出数量
...
static void write_packet(OutputFile *of, AVPacket *pkt, OutputStream *ost, int unqueue)
{
    ...
    ret = av_interleaved_write_frame(s, pkt);   // 真正写入
    if (ret < 0) {
...
static int process_input_packet(InputStream *ist, const AVPacket *pkt, int no_eof)
{
    ...
    while (ist->decoding_needed) {      // 是否重新解码编码
...
static int init_input_stream(int ist_index, char *error, int error_len)
{
    ...
    if (ist->decoding_needed) { // 是否需要解码
        ...
        ist->dec_ctx->thread_safe_callbacks = 1;    // 是否支持多线程操作 get_buffer

        av_opt_set_int(ist->dec_ctx, "refcounted_frames", 1, 0);    // 解码后的frame由调用者去释放数据
        if (ist->dec_ctx->codec_id == AV_CODEC_ID_DVB_SUBTITLE &&
           (ist->decoding_needed & DECODING_FOR_OST)) {     // 如果是字幕操作
...
static int check_init_output_file(OutputFile *of, int file_index)
{
    ...
    for (i = 0; i < of->ctx->nb_streams; i++) {
        ...
        while (av_fifo_size(ost->muxing_queue)) {   // 队列
            AVPacket pkt;
            av_fifo_generic_read(ost->muxing_queue, &pkt, sizeof(pkt), NULL);
...
static int init_output_stream(OutputStream *ost, char *error, int error_len)
{
    ...
    if (ost->encoding_needed) { // 如果需要重新编码
        ...
        // 将编码器的信息拷贝到stream的codecpar，以便写复用的时候使用
...
#endif
    return av_read_frame(f->ctx, pkt); // 先关注这里的逻辑
}
