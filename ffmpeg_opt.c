...
static void add_input_streams(OptionsContext *o, AVFormatContext *ic)
{
    ...
    for (i = 0; i < ic->nb_streams; i++) {
        ...
        GROW_ARRAY(input_streams, nb_input_streams);// nb_input_streams++操作
        input_streams[nb_input_streams - 1] = ist;  // 指向流信息

        ist->st = st;
        ist->file_index = nb_input_files;   // 起始也是从0开始
        ist->discard = 1;
...
{
    // 来将命令行中的参数通过分析分别存入此结构体的对应变量中。之后argc, argv就没用了
    OptionParseContext octx;    // 存储解析命令行参数
    uint8_t error[128];
    int ret;

    memset(&octx, 0, sizeof(octx));

    /* split the commandline into an internal representation 这里只是分隔命令行 */
    ...
    /* apply global options 解析全局变量 */
    ret = parse_optgroup(NULL, &octx.global_opts);  // 1. global_options：全局参数
    if (ret < 0) {
    ...
    /* open input files 输入的时候也会与输入相关的命令行 */
    ret = open_files(&octx.groups[GROUP_INFILE], "input", open_input_file); // 2.input_file_options：输入文件相关参数
    if (ret < 0) {
        av_log(NULL, AV_LOG_FATAL, "Error opening input files: ");
        goto fail;
    }

    /* create the complex filtergraphs 过滤器相关*/
    ...
    /* open output files 输出的时候也会与输出相关的命令行*/
    // 3. output_file_options：输出文件相关参数
